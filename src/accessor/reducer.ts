...
export const Guards = {  
    isAction(action : any) : action is Action {
        ...
        return asAction.config !== undefined && asAction.key !== undefined && asAction.base !== undefined;
    },
...
function defaultState(config : Config) : State {
    switch (config.type) {        
        case DataType.RECORDSET: return { metadata: {}, records: {} }
        default:
            throw new Error(`Can't create default state for ${config.type}`);
    }
}

export function reduce(state: any = {}, action: Action) : any {
    if (Guards.isAction(action)) {
        let base : State = getBase(state, action.base) ?? defaultState(action.config);
        ...
        switch (action.type) {
            ...
            case ActionType.updateValue:
                if (!Guards.isValueAction(action)) throw new TypeError("wrong type for action");
                editor.updateAt(action.key, action.value);
                break;             
