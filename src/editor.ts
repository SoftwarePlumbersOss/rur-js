import { State, Primitive, Record, Recordset, IRecordset, Guards, FieldMapping, Field, IMetadataCarrier, ChildMetadata } from './state';
...
export abstract class StateEditor<T extends State> {
    ...
    abstract getState() : T;
    abstract getMetadata() : IMetadataCarrier | undefined;
    ...
    editAt(path : Key, editOperation: (editor : StateEditor<State>) => void) : this {
        if (path.length > 0) {
            ...
            if (childMetadata && !isEmpty(childMetadata)) this.mergeMetadata( { childMetadata: { [head] : childMetadata } } );
        } else {
            editOperation(this);
        }
        return this;
    ...
    getMetadataAt(key: Key) : Primitive | undefined {
...
abstract class BaseStateEditor<T extends State> extends StateEditor<T> {
    ...
    getMetadata() : IMetadataCarrier | undefined {
...
class RecordsetEditor extends BaseStateEditor<Recordset> {
    ...
    mergeMetadata(metadata: IMetadataCarrier) : this {
        ...
        if (Guards.isIRecordset(this.state)) {
            this.state = { ...this.state, metadata: { ...this.state.metadata, ...metadata.metadata } } 
        } else {
            this.state = { records: this.state, metadata: metadata.metadata }
        }
        return this;
    }  
...
class PrimitiveEditor extends BaseStateEditor<Primitive> {
    ...
    constructor(config: Config, state: Primitive, metadata?: IMetadataCarrier) {
        super(config, state);
...
class FieldMappingEditor extends BaseStateEditor<FieldMapping> {
    ...
    constructor(config: Config, state: FieldMapping, metadata?: IMetadataCarrier) {
        super(config, state);
        this.metadata = metadata;
    }    
    ...
    mergeMetadata(metadata: IMetadataCarrier) : this {
        ...
        if (sourceChildMetadata) {
            for (const fieldName of new Set([...Object.keys(this.state), ...Object.keys(sourceChildMetadata)])) {
                let childEditor = this.getEditor(fieldName);
                childEditor.mergeMetadata(sourceChildMetadata[fieldName]);
                resultFields[fieldName] = childEditor.getState() as Field;
                let child = childEditor.getMetadata();
                if (child) childMetadata[fieldName] = child ;
            }
        }
        this.metadata = mergeIMetadataCarrier(this.metadata, { ...metadata, childMetadata });
        return this;
    }    
